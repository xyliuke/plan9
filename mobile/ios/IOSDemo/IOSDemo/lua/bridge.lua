...
function native:set_platform(param)
    ...
    local count = 0
    lua_c_bridge:call_native("create_timer", {interval = 1000, repeat_mode = true}, function(data)
        lua_c_bridge:log_i("timer data " .. lua_c_bridge:tostring(data))
        count = count + 1
        if count > 10 then
            lua_c_bridge:call_native("cancel_timer", {timer_id = data.result.data.timer_id})
        end
    end)
end
