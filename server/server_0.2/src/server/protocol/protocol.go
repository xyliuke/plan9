...
//直接将所有数据填充到指定位置,不对数据进行任何处理
func NewRawProtocol(id int, version byte, serverType byte, dataType byte, dataLen int, data []byte) (bool, []byte) {
	if (data != nil && dataLen <= len(data)) || (dataLen == 0 && data == nil) {
...
func RemoveFristProtocol(oriData []byte, oriDataLen int) (destDataLen int) {
	if len(oriData) >= PROTOCOL_HEADER_LEN && oriDataLen >= PROTOCOL_HEADER_LEN && oriData[0] == PROTOCOL_FRIST_LETTER {
		plen := getDataLength(oriData)
...
func IsPing(data []byte, dataLen int) bool {
	if len(data) >= PROTOCOL_HEADER_LEN && dataLen >= PROTOCOL_HEADER_LEN && data[0] == PROTOCOL_FRIST_LETTER {
		pLen := getDataLength(data)
		if (pLen + PROTOCOL_HEADER_LEN) <= dataLen {
			return IsPingByType(data[PROTOCOL_TYPE_INDEX])
		}
	}
	return false
}
...
func IsPong(data []byte, dataLen int) bool {
	if len(data) >= PROTOCOL_HEADER_LEN && dataLen >= PROTOCOL_HEADER_LEN && data[0] == PROTOCOL_FRIST_LETTER {
		pLen := getDataLength(data)
		if (pLen + PROTOCOL_HEADER_LEN) <= dataLen {
			return IsPongByType(data[PROTOCOL_TYPE_INDEX])
		}
	}
	return false
}
...
func GetTypeByte(data []byte, dataLen int) byte {
	if len(data) >= PROTOCOL_HEADER_LEN && dataLen >= PROTOCOL_HEADER_LEN && data[0] == PROTOCOL_FRIST_LETTER {
		pLen := getDataLength(data)
		if (pLen + PROTOCOL_HEADER_LEN) <= dataLen {
...
func ParseRawProtocol(data []byte, dataLen int) (result bool, id int, version byte, serverType byte, dataType byte, data_len int, raw_data []byte) {
	if len(data) >= PROTOCOL_HEADER_LEN && dataLen >= PROTOCOL_HEADER_LEN && data[0] == PROTOCOL_FRIST_LETTER {
		pLen := getDataLength(data)
		if (pLen + PROTOCOL_HEADER_LEN) <= dataLen {
			tp := data[PROTOCOL_TYPE_INDEX]
			oriData := data[PROTOCOL_DATA_INDEX : PROTOCOL_DATA_INDEX + pLen]
			dt := tp & 0x0F
			return true, getID(data), data[PROTOCOL_VERSION_INDEX], tp & 0xF0, dt, pLen, oriData
		}
	}
