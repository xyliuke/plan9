...
namespace plan9 {
    ...
    public:
        ...
        void get(std::string url, std::map<std::string, std::string>* header, std::function<void(int curl_code, std::string debug_trace, long http_state, char *, size_t)> callback) {
            http_object ho;
        ...
        void download(std::string url, std::string path, std::function<void(int curl_code, std::string debug_trace, long http_state)> callback, std::function<void(double time, long downloaded, long total)> process_callback) {
            http_object ho;
...
//                curl_formadd(&form_post, &last_ptr, CURLFORM_COPYNAME, "type", CURLFORM_COPYCONTENTS, "img", CURLFORM_END);
    ...
    void easy_http::download(std::string url, std::string path, std::function<void(int curl_code, std::string debug_trace, long http_state)> callback,
                             std::function<void(double time, long downloaded, long total)> process_callback) {
        impl_->download(url, path, callback, process_callback);
    }

    void easy_http::get(std::string url, std::map<std::string, std::string> *header,
                   std::function<void(int curl_code, std::string debug_trace, long http_state, char *data, size_t len)> callback) {
        impl_->get(url, header, callback);
    }

    void easy_http::post(std::string url, std::map<std::string, std::string> *header,
                         std::map<std::string, std::string> *form_params,
